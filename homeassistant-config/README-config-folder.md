...
1. 把 `examples/` 里的内容按需合并进 `configuration.yaml`；自动化可复制 `automations.yaml.example` 为 `automations.yaml`（并确保 `configuration.yaml` 含 `automation: !include automations.yaml`）。
